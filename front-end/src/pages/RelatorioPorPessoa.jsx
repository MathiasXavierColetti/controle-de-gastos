...
export default function RelatorioPorPessoa() {
  // Listas do Backend
  ...
  const carregarDadosIniciais = async () => {
    try {
      ...
      setError(
        "Não foi possível carregar os dados iniciais. Verifique sua conexão.",
      );
    } finally {
      setLoading(false);
    }
    ...
    try {
      ...
      setError(
        err.response?.data?.message ||
          "Erro ao carregar o relatório de gastos.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const excluirGasto = async (id) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || "Não foi possível excluir o gasto.",
      );
    }
  };
  ...
  // console.log(gastos);
  console.log(gastos);
  const valorTotalGeral = gastos.reduce(
  ...
  const resumoPorUsuario = gastos.reduce((acc, item) => {
    const usuario = item.usuarioNome || "Não informado";

    if (!acc[usuario]) {
      acc[usuario] = { total: 0, quantidade: 0, itens: [] };
    }

    acc[usuario].total += Number(item.valor || 0);
    acc[usuario].quantidade += 1;
    acc[usuario].itens.push(item);

    return acc;
  }, {});

  console.log(resumoPorUsuario);
  ...
  return (
    <div className="bg-light min-vh-100 pb-5">
      ...
      <Container className="py-4">
        ...
        <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
          <div>
            ...
            <p className="text-muted mb-0">
              Gerencie e analise as despesas por grupo e categoria.
            </p>
          </div>
          <Button
        ...
        {Object.keys(resumoPorUsuario).length > 0 && (
          <div className="mb-4">
            <h5 className="fw-bold mb-3" style={{ color: "#6d28d9" }}>
              Resumo por Usuário
            </h5>

            <Row className="g-4">
              {Object.entries(resumoPorUsuario).map(([usuario, dados], idx) => {
                // Cálculo do percentual do usuário em relação ao total geral
                const percentualDoTotal =
                  valorTotalGeral > 0
                    ? (dados.total / valorTotalGeral) * 100
                    : 0;
                      ...
                      <Card.Body className="p-4">
                        ...
                        <div className="d-flex justify-content-between align-items-start mb-3">
                          ...
                          <div className="text-end">
                            <span
                              className="fs-5 fw-bold text-primary d-block"
                              style={{ color: "#7c3aed" }}
                            >
                              R${" "}
                        ...
                        <ProgressBar
                          now={percentualDoTotal}
                          style={{ height: "6px", backgroundColor: "#f1f5f9" }}
                          className="rounded-pill mb-4"
                        />

                        {/* Lista Expansível de Gastos do Usuário */}
                        <Accordion
                          defaultActiveKey="0"
                          flush
                          ...
                          <Accordion.Item eventKey="0">
                            <Accordion.Header>
                              <span className="small fw-semibold text-secondary">
                                Ver detalhes dos gastos ({dados.itens.length})
                              </span>
                            </Accordion.Header>
                            <Accordion.Body className="p-0">
                              <ul className="list-group list-group-flush small">
                                {dados.itens.map((item, itemIdx) => (
                                  <li
                                    key={item.id || itemIdx}
                                  ...
                                  >
                                    <div className="me-2">
                                      ...
                                      {item.tipoDeGastoNome && (
                                        <span
                                          className="badge bg-light text-muted border border-0 p-0 me-2"
        ...
        <Modal show={showModal} onHide={fecharModal} centered>
          ...
          <Form onSubmit={handleSubmitGasto}>
            ...
            <Modal.Footer className="border-0 pt-0">
              ...
              <Button
                type="submit"
                className="rounded-3 border-0 px-4"
                style={{ backgroundColor: "#7c3aed" }}
              >
